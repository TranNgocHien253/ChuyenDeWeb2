...
<div class="container mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <form action="{{ route('admin.slides.index') }}" method="GET" class="flex items-center ">
            ...
            <select name="order" class="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" onchange="this.form.submit()">
        ...
        <form action="{{ route('admin.slides.create')}}" class="">
            <button class="bg-purple-600 text-white py-2 px-4 rounded hover:bg-purple-700 transition duration-300">+ Thêm Slide</button>
        </form>
    </div>
    ...
    <!-- First Row -->
    <div class="flex gap-4 items-center justify-between font-bold pb-4">
        <div class="w-1/12 text-center">STT</div>
        <div class="w-3/12">Ảnh</div>
        <div class="w-6/12 break-words">Link</div>
        <div class="w-2/12 flex justify-around">Thao tác</div>
    </div>
    ...
    <nav aria-label="Page navigation example" class="flex justify-end p-4">
        <ul class="flex items-center h-8 text-sm">
            ...
            @else
            <li>
                <a href="{{ $slides->previousPageUrl() }}" class="flex gap-1 items-center justify-center px-3 h-8 leading-tight text-gray-500 hover:text-gray-700">
            ...
            @for ($i = 1; $i <= $slides->lastPage(); $i++)
                ...
                <li>
                    <a href="{{ $slides->nextPageUrl() }}" class="flex gap-1 items-center justify-center px-3 h-8 leading-tight text-gray-500 hover:text-gray-700">
                        Next
                        <svg class="w-2.5 h-2.5 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
