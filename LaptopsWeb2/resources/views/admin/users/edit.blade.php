...
<div class="container mx-auto mt-5 p-6 max-w-3xl">
    ...
    <form action="{{ route('admin.user.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="mb-6">
            ...
            <div class="mb-4">
                <img src="{{ asset($user->image) }}" alt="Current user Image" class="h-52  w-auto rounded-lg shadow" id="current-image">
            </div>
        ...
        <div class="mb-6">
            ...
            <input type="text" name="link" id="link" class="border border-gray-300 rounded-lg w-full py-2 px-4 text-gray-700 focus:ring-2 focus:ring-blue-400 focus:outline-none" required value="{{ old('link', $user->link) }}">
        </div>
        ...
        <div class="flex h-11 gap-5 justify-between items-center">
            ...
            <a href="{{ route('admin.user.index') }}" class="flex justify-center items-center bg-red-200 text-black border border-black rounded h-full w-1/2 hover:bg-red-600 transition">Cancel</a>
        </div>
    </form>
</div>
@endsection
